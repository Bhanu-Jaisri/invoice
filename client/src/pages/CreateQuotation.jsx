...
const CreateQuotation = () => {
            ...
            fetch(`${import.meta.env.VITE_API_URL}/api/quotations/${id}`, {
                ...
                .then(data => {
                    if (data.quotation_date) {
                        data.quotation_date = data.quotation_date.split('T')[0];
                    }
                    setFormData({
    ...
    // Fetch next sequence quotation number when date changes
    useEffect(() => {
        if (isProfileIncomplete || id) return;
        const fetchNextNumber = async () => {
            try {
                const dateParam = formData.quotation_date || '';
                const res = await fetch(`${import.meta.env.VITE_API_URL}/api/quotations/next-number?date=${dateParam}`, {
                    headers: { 'x-user-id': user.id }
                });
                if (res.ok) {
                    const data = await res.json();
                    setFormData(prev => ({ ...prev, quotation_number: data.nextNumber }));
                }
            } catch (err) {
                console.error('Failed to generate quotation sequence:', err);
            }
        };
        fetchNextNumber();
    }, [formData.quotation_date, user.id, isProfileIncomplete, id]);
        ...
        return (
            <div className="max-w-md mx-auto my-12 bg-white rounded-2xl shadow-md border border-gray-150 p-6 text-center">
                ...
                <p className="text-sm text-gray-500 mt-2 mb-6">
                    You must complete your business details and select a theme color before creating quotes or invoices.
                </p>
                <Link to="/profile" className="inline-block bg-primary hover:bg-secondary text-white font-bold px-6 py-2.5 rounded-xl transition-all">
                    Complete Setup
                </Link>
            </div>
        );
    ...
    const handleItemChange = (index, fieldName, value) => {
        setItems(prev => prev.map((item, idx) => {
            if (idx === index) {
                ...
                if ((fieldName === 'custom_item_type' || fieldName === 'item_category') && value) {
                    const matchingProd = savedProducts.find(p => p.name.trim().toLowerCase() === value.trim().toLowerCase());
                    if (matchingProd) {
                        if (matchingProd.hsn_sac) updated.hsn_sac = matchingProd.hsn_sac;
                        if (matchingProd.unit) updated.unit = matchingProd.unit;
                        if (matchingProd.price_per_unit) updated.price_per_unit = matchingProd.price_per_unit;
                        if (matchingProd.gst_rate !== undefined) updated.gst_rate = matchingProd.gst_rate;
                    }
                }

                if (fieldName === 'item_category') {
                    if (value !== 'Other') {
                        updated.gst_rate = 18;
                    } else {
                        updated.gst_rate = '';
                    ...
                    if (value === 'Daily Calendar') {
                        updated.designs = [{ design_number: '1', quantity: '' }];
                        updated.quantity = 0;
                        updated.designs_total_qty = 0;
                    } else {
                        updated.designs = null;
                        updated.designs_total_qty = null;
    ...
    const handleAddDesign = (itemIndex) => {
        setItems(prev => prev.map((item, idx) => {
            if (idx === itemIndex) {
                const currentDesigns = item.designs || [];
                const nextDesignNo = currentDesigns.length + 1;
                const newDesigns = [...currentDesigns, { design_number: String(nextDesignNo), quantity: '' }];
                return {
                    ...item,
                    designs: newDesigns
                };
    ...
    const handleRemoveDesign = (itemIndex, designIndex) => {
        setItems(prev => prev.map((item, idx) => {
            if (idx === itemIndex) {
                const currentDesigns = item.designs || [];
                if (currentDesigns.length <= 1) return item;
                const newDesigns = currentDesigns.filter((_, dIdx) => dIdx !== designIndex);

                // Recalculate total quantity
                const totalQty = newDesigns.reduce((sum, d) => sum + (parseFloat(d.quantity) || 0), 0);
                return {
                    ...item,
                    designs: newDesigns,
                    quantity: totalQty,
                    designs_total_qty: totalQty
                };
            }
            return item;
        }));
    ...
    const handleDesignChange = (itemIndex, designIndex, field, value) => {
        setItems(prev => prev.map((item, idx) => {
            if (idx === itemIndex) {
                const currentDesigns = (item.designs || []).map((design, dIdx) => {
                    if (dIdx === designIndex) {
                        return { ...design, [field]: value };
                    }
                    return design;
                });
                ...
                if (field === 'quantity') {
                    totalQty = currentDesigns.reduce((sum, d) => sum + (parseFloat(d.quantity) || 0), 0);
                }
    ...
    // If state is not filled, default to office state to calculate CGST/SGST
    const isSameState = !formData.customer_state || formData.customer_state.toLowerCase().trim() === (user.office_state || '').toLowerCase().trim();
    ...
    // Total quotation amount excludes GST
    ...
    const handleSubmit = async (e) => {
        ...
        // Validation for custom item details
        for (let i = 0; i < items.length; i++) {
            const item = items[i];
            if (item.item_category === 'Other' && !item.custom_item_type.trim()) {
                alert(`Please specify the custom item details for Item #${i + 1}`);
                setLoading(false);
                return;
            }
            if (item.item_category === 'Dairy' && !item.size.trim()) {
                alert(`Please enter a size for Dairy Item #${i + 1}`);
                setLoading(false);
                return;
            }
            if (parseFloat(item.quantity) <= 0 || parseFloat(item.price_per_unit) < 0) {
                alert(`Please verify quantity and rate values for Item #${i + 1}`);
                setLoading(false);
                return;
            }
        }
        ...
        try {
            ...
            const endpoint = id ? `${import.meta.env.VITE_API_URL}/api/quotations/${id}` : `${import.meta.env.VITE_API_URL}/api/quotations`;
        ...
        <form onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-6">
            <div className="bg-white p-6 md:p-8 rounded-2xl shadow-md border border-gray-100 space-y-6">
                <div className="border-b border-gray-100 pb-4 flex justify-between items-center">
                    <div>
                        <h2 className="text-xl font-bold text-gray-800">{id ? 'Edit Quotation Details' : 'Generate New Quotation'}</h2>
                        <p className="text-sm text-gray-500">Provide customer details and add item sets to generate estimate.</p>
                    </div>
                    <div className="text-right">
                        <span className="text-xs text-gray-400 font-bold block">Quotation Number</span>
                        <span className="text-sm font-bold text-primary">{formData.quotation_number || 'Generating...'}</span>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-gray-500 mb-1">Quotation Date</label>
                            <div className="relative">
                                <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-405">
                                    <FileText size={16} />
                                </span>
                            ...
                            <div key={item.id} className="p-4 md:p-6 bg-gray-50 rounded-2xl border border-gray-200 relative space-y-4 animate-fadeIn">
                                ...
                                {item.item_category === 'Daily Calendar' && (
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-white rounded-xl border border-gray-150">
                                        <div>
                                            <label className="block text-xs font-semibold text-gray-650 mb-1">Size *</label>
                                            <input
                                                required
                                                type="text"
                                ...
                                {item.item_category === 'Daily Calendar' && (
                                    <div className="p-4 bg-white rounded-xl border border-gray-150 space-y-4">
                                        ...
                                        <div className="flex justify-end text-xs font-bold text-gray-700 bg-blue-50/50 p-2 rounded-lg border border-blue-100/50">
                                            <span>Computed Total Quantity: <span className="text-primary text-sm font-extrabold">{item.quantity || 0}</span> 1</span>
                                        </div>
                                    </div>
                                )}
                                ...
                                {item.item_category === 'Monthly Calendar' && (
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-white rounded-xl border border-gray-150">
                                        ...
                                        <div>
                                            ...
                                            <select
                                                className="w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm outline-none font-semibold text-gray-700"
                                                value={item.sheeter}
                ...
                <div className="pt-6 border-t border-gray-100 flex justify-end">
                    <div className="w-full md:w-80 space-y-2">
                        ...
                        <div className="bg-amber-50 p-2.5 rounded-lg border border-amber-100 text-[11px] text-amber-800 leading-relaxed">
                            💡 <strong>GST Policy:</strong> GST is calculated separately for reference but is <strong>not added</strong> to the final quotation payable amount.
                        </div>
                        <div className="flex justify-between text-base border-t border-gray-100 pt-2 font-bold text-gray-800">
                            ...
                            <span className="text-primary text-lg">₹{total_amount.toFixed(2)}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex justify-end space-x-3 pt-4 border-t border-gray-100">
                    ...
                    >
                        <Save size={16} />
                        <span>{loading ? 'Saving...' : (id ? 'Update Quotation' : 'Save Quotation')}</span>
                    </button>
                </div>
            </div>
        </form>
