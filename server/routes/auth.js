...
router.post('/login', async (req, res) => {
    try {
        ...
        const result = await db.query(
            'SELECT id, username, password, office_name, office_address, office_gstin, office_email, office_mobile, office_state, office_theme, bank_name, bank_branch, account_name, account_no, ifsc_code, terms_conditions FROM users WHERE username = $1',
...
router.get('/profile', async (req, res) => {
    try {
        ...
        const result = await db.query(
            'SELECT id, username, office_name, office_address, office_gstin, office_email, office_mobile, office_state, office_theme, bank_name, bank_branch, account_name, account_no, ifsc_code, terms_conditions FROM users WHERE id = $1',
            [parseInt(userId, 10)]
        );
...
router.put('/profile', async (req, res) => {
    try {
        ...
        if (!office_name || !office_address || !office_gstin || !office_email || !office_mobile || !office_state) {
            return res.status(400).json({ error: 'All primary fields are required to update office profile' });
        }
        ...
        const result = await db.query(
            `UPDATE users SET 
                ...
                terms_conditions = $13
             WHERE id = $14 
             RETURNING id, username, office_name, office_address, office_gstin, office_email, office_mobile, office_state, office_theme, bank_name, bank_branch, account_name, account_no, ifsc_code, terms_conditions`,
            [
                office_name.trim(),
                office_address.trim(),
                office_gstin.trim().toUpperCase(),
                office_email.trim(),
                office_mobile.trim(),
                office_state.trim(),
                theme,
